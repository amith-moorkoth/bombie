...
> project hosted on Vercel for show-and-tell. Several features are partial
> (e.g. Upload / Download JSON in the toolbar are still placeholders), the
> dependency tree is still being trimmed, and breaking changes can land
...
The live demo at **https://bombie-three.vercel.app/** is hosted on **Vercel**.

### Vercel (primary)

Config lives in [vercel.json](vercel.json) at the repo root. SPA deep links
(`/generate-component`, anything else) are restored by a single rewrite
rule pointing unknown paths back at `/index.html`. Long-cache headers are
set for the content-hashed `js/`, `styles/`, and `assets/` directories so
the CDN serves them immutably.

To deploy your own fork to Vercel:

1. Sign in at **vercel.com** with GitHub.
2. **Add New → Project → Import** your fork.
3. Vercel auto-detects the config from `vercel.json`. Defaults work
   (`npm run build` + output `dist`).
4. No env vars required for a root-domain deploy — `PUBLIC_URL_PATH`
   defaults to `/` (see [config/webpack.common.js](config/webpack.common.js)).
5. Push to `main` and Vercel auto-deploys.

### GitHub Pages (alternative)

[.github/workflows/deploy.yml](.github/workflows/deploy.yml) builds with
`PUBLIC_URL_PATH=/bombie/` and publishes via `actions/deploy-pages@v4` on
every push to `main`. You can also publish manually with:
...
[public/spa-redirect.js](public/spa-redirect.js) restore SPA deep links on
GitHub Pages (which doesn't have rewrites) via a query-string round-trip
that React Router resolves on load.

To enable for your fork:

1. Repo **Settings → Pages → Source** = `GitHub Actions`.
2. Update `PUBLIC_URL_PATH` in `.github/workflows/deploy.yml` to match
   your repo name.
3. Push to `main` (or run the **Deploy to GitHub Pages** workflow
   manually). Live at `https://<your-username>.github.io/<your-repo>/`.
