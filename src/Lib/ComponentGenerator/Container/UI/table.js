...
export const UI_Table = memo(function Container({
  currentChild,
  handleonDrop,
  handleonDrop_Move,
  handleonHover_Move,
    ...
    >
      ...
      <PropertyController currentChild={currentChild} formData={formData}>
        <TextField
          ...
          label="minWidth"
          size="small"
          value={getFromState(formData, "minWidth")}
          onChange={(e) => {
            setformData(updateToState(formData, "minWidth", e.target.value));
          }}
        />
      </PropertyController>
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: currentChild?.props?.minWidth }}>
          {children}
        </Table>
      </TableContainer>
    </DropBox>
