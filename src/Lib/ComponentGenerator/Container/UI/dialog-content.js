...
export const UI_DialogContent = memo(function Container({
  currentChild,
  handleonDrop,
  handleonDrop_Move,
  handleonHover_Move,
      ...
      >
        ...
        <PropertyController currentChild={currentChild} formData={formData}>
          <TextField
            ...
            size="small"
            value={getFromState(formData, "dialogContentText")}
            onChange={(e) => {
              setformData(
                updateToState(formData, "dialogContentText", e.target.value)
              );
            }}
          />
        </PropertyController>
        <DialogContentText>
          {currentChild?.props?.dialogContentText}
        </DialogContentText>
        {children}
      </DropBox>
    </CardContent>
